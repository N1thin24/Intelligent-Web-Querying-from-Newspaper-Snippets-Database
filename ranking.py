...
class Ranking:
    ...
    def compute_score(self, query, docid, index_trie, tfidf_query):
        ...
        if self.choice == 1:
            final_score = tot_score / (math.sqrt(length_tq) * math.sqrt(length_td)) #1. Cosine Similarity                                                                                  
        elif self.choice == 2:  #2. Summation of tf-idf scores w.r.t document
            final_score = doc_tot_score 
        elif self.choice == 3:     #3. Summation of (tf-idf w.r.t doc * tf-idf w.r.t query)  
